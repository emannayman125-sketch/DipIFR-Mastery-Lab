...
// ---------------------------------------------------------------------------
// Shared answer UI (textarea / spreadsheet toggle, "Ask the AI Tutor about
// this question", Submit & Mark, and the score/feedback result) — used by
// both adaptive Practice (one question at a time, "Next question →") and
// the Question Bank (pick any specific question, answer it the same way).
// ---------------------------------------------------------------------------
...
function QuestionAnswerPanel({
  ...
  // Reset the answer whenever a different question is shown — either
  // "Next question" in adaptive Practice, or picking another Question
  // Bank item — so a previous answer never leaks onto a new question.
  useEffect(() => {
    setAnswer("");
  ...
  return (
    <div className="question">
      <small>
        {question.topic_code}
        {question.related_standards.length > 1 &&
          ` (+ ${question.related_standards.filter(c => c !== question.topic_code).join(", ")})`}{" "}
      ...
      {question.source_round && (
        <div className="sourceTag">
          ...
          {question.source === "past_exam"
            ? `Real ACCA past exam — ${question.source_round}${question.question_number ? ` · Q${question.question_number}` : ""}`
            : question.source_round}
        </div>
      )}
      ...
      <button
        ...
        onClick={() =>
          onAskTutor(
            `${question.prompt}\n\n(Marks: ${question.marks}, Standard(s): ${question.related_standards.join(", ")})`
          )
        }
      >
      ...
      {!result && (
        <div className="examHeader" style={{ marginTop: 8 }}>
          ...
          <button
            type="button"
            className="ghost"
            onClick={() => setMode(m => (m === "word_processor" ? "spreadsheet" : "word_processor"))}
          >
      ...
      {mode === "word_processor" ? (
        <textarea
          ...
          placeholder="Write your exam-style answer..."
          disabled={!!result}
        />
      ) : (
      ...
      {!result ? (
        <button className="primary" onClick={submit} disabled={saving || !answer.trim()}>
          {saving ? "Marking…" : "Submit & Mark"}
        </button>
      ) : (
        <>
          <div className="success">
            Score: {result.score}%. {result.feedback}{" "}
            {!result.aiGraded && <em>(keyword-based grading — connect an AI provider for richer feedback)</em>}
          </div>
          {onNext && (
...
function QuestionBank({ onSubmitted, onAskTutor }: { onSubmitted: () => void; onAskTutor: (context: string) => void }) {
  ...
  const [selected, setSelected] = useState<import('./lib/api').QuestionBankItem | null>(null);
        ...
        <div className="panel">
          ...
          <QuestionAnswerPanel
            question={{
              ...
              topic_code: selected.standard_code,
              related_standards: selected.related_standards,
