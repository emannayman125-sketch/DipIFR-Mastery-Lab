...
- Email delivery (`backend/app/core/email.py`) supports real SMTP providers (Amazon SES, Postmark, Mailgun, Gmail/Workspace, etc.) and a dedicated SendGrid HTTPS-API path, selected automatically once `SMTP_HOST` and `SMTP_FROM_EMAIL` are set. **Until those environment variables are configured, the app falls back to `ConsoleEmailSender`**, which only logs verification/reset emails instead of delivering them — real users will not receive those links until a provider is configured. `validate_runtime()` refuses to start with `ENVIRONMENT=production` unless `SMTP_HOST`/`SMTP_FROM_EMAIL` are set, specifically to prevent shipping with emails silently undelivered.
